...
export async function syncTransactions(
  fromBudget: BudgetConfig,
  toBudget: BudgetConfig,
  ...
  const mapToTransaction = (
    ...
    if (
      syncConfig.transactions.includeTransfers == true &&
    ...
    return {
      ...
      subtransactions: transaction.subtransactions
        ...
        ?.filter((transaction) =>
          isRelevantTransaction(transaction, syncConfig)
        ),
...
async function getTransactions(
  ...
  if (syncConfig.transactions.excludeImported == true) {
    return transactions.filter(
      (transaction) => transaction.imported_id == null
    );
  }
  return transactions;
}

async function getPayeeMap(
  fromPayees: APIPayeeEntity[]
): Promise<Map<string, APIPayeeEntity>> {
...
async function getCategoryMap(
  syncConfig: SyncConfig,
  fromCategoryGroups: APICategoryGroupEntity[],
  toCategoryGroups: APICategoryGroupEntity[]
): Promise<Map<string, CategoryMapEntry | undefined>> {
  ...
  for (const fromCategoryGroup of fromCategoryGroups) {
    const toCategoryGroup = toCategoryGroups.find(
      (group) => group.name === fromCategoryGroup.name
    );

    if (
      syncConfig.categories.excludeGroups &&
      syncConfig.categories.excludeGroups.includes(fromCategoryGroup.name)
    ) {
      continue;
    }
    ...
    if (fromCategoryGroup.categories) {
      for (const fromCategory of fromCategoryGroup.categories) {
        const toCategory = toCategoryGroup?.categories?.find(
          (category) => category.name === fromCategory.name
        );
...
function isRelevantTransaction(
  ...
  if (transaction.category == null) {
    if (transaction.subtransactions && transaction.subtransactions.length > 0) {
      return transaction.subtransactions.some((subtransaction) =>
        isRelevantTransaction(subtransaction, syncConfig)
      );
    } else {
      return false;
    }
